...
class DraggableButton(Button, EventDispatcher):
    double_click_interval = 0.3  # ダブルクリックと認識するタップ間隔（秒）
...
class MainApp(App):
    def build(self):
        ...
        button1.bind(on_double_click=self.on_button_double_click)  # ボタン1のダブルクリック時の処理を設定
    ...
    def on_button_double_click(self, instance):
        print(f"Double-clicked on button: {instance.text}")  # ダブルクリック時の処理を追加
